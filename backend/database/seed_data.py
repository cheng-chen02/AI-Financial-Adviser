...
def main():
    print("🚀 Seeding Instrument Data".encode("utf-8"))
    print("=" * 50)
    print(f"Loading {len(INSTRUMENTS)} instruments...")

    # First verify all allocations
    print("\n📊 Verifying allocation data...".encode("utf-8"))
    all_valid = True
    ...
    print("  ✅ All allocations valid!".encode("utf-8"))

    # Insert instruments
    print("\n💾 Inserting instruments...".encode("utf-8"))
    success_count = 0
    ...
    for inst in INSTRUMENTS:
        ...
        if insert_instrument(inst):
            print(f"    ✅ Success".encode("utf-8"))
            success_count += 1
        else:
            print(f"    ❌ Failed")

    print("\n" + "=" * 50)
    print(f"Seeding complete: {success_count}/{len(INSTRUMENTS)} instruments loaded".encode("utf-8"))

    # Verify by querying
    print("\n🔍 Verifying data...".encode("utf-8"))
    try:
        response = client.execute_statement(
            resourceArn=cluster_arn,
            secretArn=secret_arn,
        ...
        print(f"  Database now contains {count} instruments".encode("utf-8"))
        ...
        print("\n  Sample instruments:".encode("utf-8"))
        for record in response["records"]:
            symbol = record[0]["stringValue"]
            name = record[1]["stringValue"]
            print(f"    - {symbol}: {name}".encode("utf-8"))

    except ClientError as e:
        print(f"  ❌ Error verifying: {e}")

    print("\n✅ Seed data loaded successfully!".encode("utf-8"))
    print("\n📝 Next steps:".encode("utf-8"))
    print("1. Create test user and portfolio: uv run create_test_data.py".encode("utf-8"))
    print("2. Test database operations: uv run test_db.py".encode("utf-8"))
