...
# Only initialize if CLERK_JWKS_URL is set
clerk_jwks_url = os.getenv("CLERK_JWKS_URL")
if clerk_jwks_url:
    clerk_config = ClerkConfig(jwks_url=clerk_jwks_url)
    clerk_guard = ClerkHTTPBearer(clerk_config)
else:
    logger.warning("CLERK_JWKS_URL not set - authentication disabled for local development")
    clerk_guard = None

async def get_current_user_id(creds: HTTPAuthorizationCredentials = Depends(clerk_guard) if clerk_guard else None) -> str:
    """Extract user ID from validated Clerk token"""
    if not clerk_guard:
        # Local development mode - use test user
