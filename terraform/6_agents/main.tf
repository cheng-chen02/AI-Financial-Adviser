...
resource "aws_iam_role_policy" "lambda_agents_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # Bedrock access for all agents (multiple regions for inference profiles)
      {
        Effect = "Allow"
        Action = [
          "bedrock:InvokeModel",
...
# Additional policy for cross-region Bedrock access
resource "aws_iam_role_policy" "lambda_bedrock_cross_region" {
  name = "alex-lambda-bedrock-cross-region"
  role = aws_iam_role.lambda_agents_role.id
